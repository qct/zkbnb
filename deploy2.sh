...
BSC_TESTNET_RPC=http://127.0.0.1:8545 # https://data-seed-prebsc-1-s1.binance.org:8545
...
#python3 verifier_parse.py ${KEY_PATH}/ZkBNBVerifier1.sol 1 ${DEPLOY_PATH}/zkbnb-contract/contracts/ZkBNBVerifier.sol
...
[[ "${blockNumber}" -ne "0" ]] || blockNumber=1
...
echo -e "
go run ./cmd/zkbnb/main.go prover --config ${DEPLOY_PATH}/zkbnb/service/prover/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6060 --metrics --metrics.addr localhost --metrics.port 6060
" > run_prover.sh
...
echo -e "
go run ./cmd/zkbnb/main.go witness --config ${DEPLOY_PATH}/zkbnb/service/witness/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6061 --metrics --metrics.addr localhost --metrics.port 6061
" > run_witness.sh
# remove the fist line if it includes -e
...
echo -e "
go run ./cmd/zkbnb/main.go monitor --config ${DEPLOY_PATH}/zkbnb/service/monitor/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6062 --metrics --metrics.addr localhost --metrics.port 6062
" > run_monitor.sh
# remove the fist line if it includes -e
...
echo -e "
go run ./cmd/zkbnb/main.go committer --config ${DEPLOY_PATH}/zkbnb/service/committer/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6063 --metrics --metrics.addr localhost --metrics.port 6063
" > run_committer.sh
# remove the fist line if it includes -e
...
echo -e "
go run ./cmd/zkbnb/main.go sender --config ${DEPLOY_PATH}/zkbnb/service/sender/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6064 --metrics --metrics.addr localhost --metrics.port 6064
" > run_sender.sh
# remove the fist line if it includes -e
...
echo -e "
go run ./cmd/zkbnb/main.go apiserver --config ${DEPLOY_PATH}/zkbnb/service/apiserver/etc/config.yaml --pprof --pprof.addr localhost --pprof.port 6065 --metrics --metrics.addr localhost --metrics.port 6065
" > run_apiserver.sh
# remove the fist line if it includes -e
...
#pm2 start --name apiserver "/app/run_apiserver.sh"
#
##1. deploy zkbnb
#stop all
#clear database
#clear redis
#clear kvrocks
#drop database if exists zkbnb-test
#
